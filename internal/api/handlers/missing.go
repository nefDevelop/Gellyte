...
func GetShowEpisodes(c *gin.Context) {
	...
	var total int64
	query := database.DB.Model(&models.MediaItem{}).Where("type = ? AND parent_id = ?", "Episode", seriesId)
...
func GetShowSeasons(c *gin.Context) {
	// Gellyte no tiene modelo de Season actualmente, por lo que devolvemos un mock vacío o 
	// podríamos inferir Seasons desde los episodios en el futuro.
	c.JSON(http.StatusOK, gin.H{
		"Items": []gin.H{},
		"TotalRecordCount": 0,
	})
